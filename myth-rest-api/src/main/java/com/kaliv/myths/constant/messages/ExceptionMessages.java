package com.kaliv.myths.constant.messages;

public class ExceptionMessages {
    public static final String RESOURCE_WITH_GIVEN_VALUE_NOT_FOUND = "%s with %s '%s' not found";
    public static final String RESOURCE_NOT_FOUND = "%s not found";
    public static final String RESOURCE_LIST_NOT_FOUND = "%s with given %s not found";
    public static final String RESOURCE_WITH_GIVEN_VALUE_ALREADY_EXISTS = "%s with %s '%s' already exists";
    public static final String RESOURCE_ALREADY_EXISTS = "%s already exists";
    public static final String DUPLICATE_ENTRIES = "Duplicate entries in given %s and %s";
    public static final String INVALID_PARENT = "Myth character cannot be a parent of themselves";
    public static final String INVALID_ARTWORK_TYPE = "Invalid visual artwork type";
    public static final String INVALID_IMAGE = "Invalid or empty image";
    public static final String UNSUPPORTED_IMAGE_CONTENT_TYPE = "Unsupported image content type";
}
